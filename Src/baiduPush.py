...
# 
# Modified base on LoveNight version on 2016/1/7 
#
...
url="http://data.zz.baidu.com/urls?site=blog.shuiguzi.com&token=r1VgqJ5wOxxJsyQD&type=original"
    ...
    for _ in sitemap:
        if os.path.exists(_):
            with open(_, "r") as f:
                xml = f.read()
                xmlDict = parse(xml);
                for item in xmlDict["urlset"]["url"]:
                    page_url = item["loc"]
                    if page_url:
